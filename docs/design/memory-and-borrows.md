...
- **M12** A borrowed operand that is not a place is given one: a temporary is an anonymous
  ...
  const evaluator address it, the editor never sees it). The root is what is materialized:
  `mk().a.&mut` borrows into the temporary's field, not a copy of it. Storage death is
...
- **Refusing `.&raw`/`.&raw mut` of a temporary** — the original reason was that a raw pointer
  carries no region, so nothing would catch it outliving storage minted for it; overridden
  (owner, 2026-09-10) because that is equally true of a raw pointer to a block-local, which
  already checks clean and is priced at the deref, under `unsafe` (D1). Refusing the temporary
  case bought no safety the local case did not already forgo. **M12**
