...
/// `.&raw` of a temporary materializes it like `.&` does: the program
/// checks clean, and `mkp().a.&raw mut` points into the temporary's own
/// field, not a copy of it. A raw pointer to a temporary dangles at block
/// end exactly like one to a named local, which `unsafe` at the deref
/// already prices.
#[test]
