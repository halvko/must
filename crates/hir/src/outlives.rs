...
//!
//! This module has no storage liveness, and neither has MIR or the
//! interpreter. A materialized temporary ([`crate::body::Body::temps`],
//! M12) lives to the end of its innermost enclosing block by rule, but a
//! borrow that outlives it inside one body is caught by nothing, the same
//! hole a named block-local has. What is fenced is escape: a borrow rooted
//! in a temporary roots in body-local storage, so a region forced past the
//! body is [`OutlivesDiagnostic::BorrowEscapes`], worded to name the
//! temporary because the user wrote no `let` to look for.
