...
pub fn evaluate(
    ...
    // The entry is the appended item, picked by position so a user static
    // that happens to share the name can't hijack `-e`.
    let Some(&entry) = hir::file_item_ids(&db, file).iter().find(|&&item| {
