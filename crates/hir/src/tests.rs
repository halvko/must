...
fn an_import_declaration_has_an_unsafe_fn_type() {
    ...
    check_infer(
        ...
        expect![[r#"
            89..117 'fn() -> () { let ...': fn()
            100..117 '{ let f = read; }': ()
            106..107 'f': unsafe fn(u8.&raw mut, usize) -> i64
            110..114 'read': unsafe fn(u8.&raw mut, usize) -> i64
        "#]],
    );
}
...
fn an_imports_unwritten_return_type_is_unit() {
    ...
    check_infer(
        "unsafe extern static tick: unsafe fn(n: i64);\n\
         static held = fn() -> () { let f = tick; };",
        expect![[r#"
            60..88 'fn() -> () { let ...': fn()
            71..88 '{ let f = tick; }': ()
            77..78 'f': unsafe fn(i64)
            81..85 'tick': unsafe fn(i64)
        "#]],
    );
}
...
fn taking_an_import_as_a_value_is_free_and_its_call_is_gated() {
    ...
    check_diagnostics(
        ...
        expect![[r#"
            181..188: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn a_vouched_safe_import_is_called_with_no_marker_at_all() {
    ...
    check_diagnostics(
        "unsafe extern static now: fn() -> i64;\n\
         ...
         static bound = fn() -> i64 { let f = now; f() };",
        expect![[""]],
    );
}
...
fn an_unsafe_fn_does_not_coerce_to_a_safe_fn() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            210..214: type mismatch: expected `fn(u8.&raw mut, usize) -> isize`, found `unsafe fn(u8.&raw mut, usize) -> isize` (expected `fn(u8.&raw mut, usize) -> isize` because of this annotation at 176..207)
        "#]],
    );
}
...
fn calling_a_let_bound_unsafe_fn_needs_the_marker() {
    check_unsafe_fn(
        ...
        expect![[r#"
            200..207: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn calling_an_unsafe_fn_returned_from_a_function_needs_the_marker() {
    check_unsafe_fn(
        ...
        expect![[r#"
            257..269: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn calling_an_unsafe_fn_out_of_a_record_field_needs_the_marker() {
    check_unsafe_fn(
        ...
        expect![[r#"
            216..226: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn the_conversion_is_shallow_but_a_literal_is_checked_per_part() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            407..408: type mismatch: expected `struct { go: unsafe fn(u8.&raw mut, usize) -> isize }`, found `struct { go: fn(u8.&raw mut, usize) -> isize }` (expected `struct { go: unsafe fn(u8.&raw mut, usize) -> isize }` because of this annotation at 351..404)
            516..517: type mismatch: expected `fn() -> unsafe fn(usize) -> usize`, found `fn() -> fn(usize) -> usize` (expected `fn() -> unsafe fn(usize) -> usize` because of this annotation at 480..513)
        "#]],
    );
}
...
fn a_safe_fn_parameter_refuses_an_import() {
    ...
    check_unsafe_fn(
        "static apply = fn(g: fn(u8.&raw mut, usize) -> isize, p: u8.&raw mut) -> isize { \
         ...
         static f = fn(p: u8.&raw mut) -> isize { apply(read, p) };",
        expect![[r#"
            284..288: type mismatch: expected `fn(u8.&raw mut, usize) -> isize`, found `unsafe fn(u8.&raw mut, usize) -> isize`
        "#]],
    );
}
...
fn a_generic_instantiated_at_an_unsafe_fn_keeps_the_price() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            224..238: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn an_item_level_unsafe_fn_annotation_lowers_the_same() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            255..269: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn a_join_of_imports_is_an_unsafe_fn_and_its_call_is_gated() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            246..253: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn a_match_join_and_a_loop_break_join_mint_the_same_way() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            258..267: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
            392..401: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn a_join_mixing_a_safe_fn_and_an_import_settles_on_the_unsafe_one() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            251..258: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn a_chain_of_pending_joins_is_followed_to_its_unsafe_witness() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            286..293: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
            446..453: calling a value of `unsafe fn` type requires an `unsafe { ... }` block
        "#]],
    );
}
...
fn an_unconsumed_mixed_safety_join_is_a_branch_disagreement() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            205..214: `if` branches have incompatible types: `unsafe fn(u8.&raw mut, usize) -> isize` vs `fn(u8.&raw mut, usize) -> isize`; add a type annotation to decide between them (this branch has type `unsafe fn(u8.&raw mut, usize) -> isize` at 191..195)
        "#]],
    );
}
...
fn a_join_still_refuses_an_import_where_a_safe_fn_is_demanded() {
    ...
    check_unsafe_fn(
        ...
        expect![[r#"
            224..228: type mismatch: expected `fn(u8.&raw mut, usize) -> isize`, found `unsafe fn(u8.&raw mut, usize) -> isize` (expected `fn(u8.&raw mut, usize) -> isize` because of this annotation at 183..214)
        "#]],
    );
}
...
fn a_prefix_operand_never_reports_a_token_outside_its_own_expression() {
    ...
    // diagnostic below is the retired prefix spelling, unrelated; the
    // assertion is that the lint says NOTHING here.
    check_diagnostics(
        r#"
static f = fn () -> () {
    let x: usize = 1;
...
/// value got storage, as `range 'snippet'`.
...
/// exclusively, straight into a member that writes through it, checks
/// clean.
#[test]
...
/// The mark lands on the ROOT of the borrowed place: field and element
/// roots, a receiver-position temporary, and a nested one.
#[test]
...
/// A deref-rooted place gets no temporary: `p.*.x` names storage the
/// pointer already points at.
#[test]
...
/// A must-consume value cannot be borrowed as a temporary: a temporary
/// has no name, so nothing could ever consume it.
#[test]
...
/// `.&raw` of a temporary checks clean and marks the root, so
/// `mkp().a.&raw mut` points into the temporary's own field.
#[test]
...
/// `.&raw` of a must-consume temporary gets the bind-it-with-`let`-first
/// refusal.
#[test]
...
/// A borrowed name is never materialized, whatever it resolves to.
/// TODO: a builtin is a function value and should borrow (halvko/must#31).
#[test]
...
static raw_param = fn::<const N: usize>() -> () { let p = N.&raw; };
static raw_builtin = fn() -> () { let p = print.&raw mut; };
"#,
        expect![[r#"
            ...
            336..342: `.&raw` cannot take the address of `N`: it is a const parameter, not a value with storage
            389..403: `.&raw` cannot take the address of `print`: it is a builtin, not a value with storage
        "#]],
    );
}
